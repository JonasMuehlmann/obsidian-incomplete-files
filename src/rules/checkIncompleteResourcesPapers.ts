...
export const checkIncompleteResourcesPapers: IssueScanner = {
	...
	issueType: INCOMPLETE_ISSUE_TYPE.INCOMPLETE_PAPER,
	setting: {
		name: "Incomplete paper",
		description: "Check if the paper has incomplete properties",
		default: true,
	},
    func: (file: TFile, data: Data) => {
        
        if (file.path.contains("Resources/Papers/") == false)
        {
            return [];
        }
        ...
        const title: string = "Incomplete paper; ";

        const authors: string[] = yamlObj.author;
        if (authors == null || authors.length == 0)
        {
            issues.push({
                type: INCOMPLETE_ISSUE_TYPE.INCOMPLETE_PAPER,
        ...
        const journal: string = yamlObj["journal of publication"];
        if (journal == null)
        {
            issues.push({
                type: INCOMPLETE_ISSUE_TYPE.INCOMPLETE_PAPER,
                title: title + "no journal",
            })
        }

        const link: string = yamlObj["link"];
        const doi: string = yamlObj["DOI"];
        if (link == null && doi == null)
        {
            issues.push({
                type: INCOMPLETE_ISSUE_TYPE.INCOMPLETE_PAPER,
                title: title + "no link or DOI",
            })
        }
