...
export const checkIncompleteResourcesVideos: IssueScanner = {
	...
	issueType: INCOMPLETE_ISSUE_TYPE.INCOMPLETE_VIDEO,
	setting: {
		name: "Incomplete video",
		description: "Check if the video has incomplete properties",
		default: true,
	},
    func: (file: TFile, data: Data) => {
        
        if (file.path.contains("Resources/Videos/") == false)
        {
            return [];
        }
        ...
        const title: string = "Incomplete video; ";

        const authors: string[] = yamlObj.author;
        if (authors == null || authors.length == 0)
        {
            issues.push({
                type: INCOMPLETE_ISSUE_TYPE.INCOMPLETE_VIDEO,
